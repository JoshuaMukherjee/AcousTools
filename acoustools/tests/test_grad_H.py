from acoustools.Mesh import load_scatterer, translate, get_centres_as_points
...
Hx, Hy, Hz, Hx_old, Hy_old, Hz_old = grad_H(None, ball, board,path=path, use_cache_H=cache, H=H)
...
grad_H_x = (pressure_dx_right - pressure_dx_left) / 2*h

Px_fd = torch.abs(grad_H_x).squeeze()
Px_an = torch.abs(Hx@x).squeeze()
Px_an_old = torch.abs(Hx_old@x).squeeze()

print(torch.stack([Px_fd, Px_an, Px_fd/Px_an, Px_an_old]))


ball_dy_left = ball.clone()
...
grad_H_y = (pressure_dy_right - pressure_dy_left) / 2*h
Py_fd = torch.abs(grad_H_y).squeeze()
Py_an = torch.abs(Hy@x).squeeze()
Py_an_old = torch.abs(Hy_old@x).squeeze()

print(torch.stack([Py_fd, Py_an, Py_fd/Py_an, Py_an_old]))



ball_dz_left = ball.clone()
...
grad_H_z = (pressure_dz_right - pressure_dz_left) / 2*h
Pz_fd = torch.abs(grad_H_z).squeeze()
Pz_an = torch.abs(Hz@x).squeeze()
Pz_an_old = torch.abs(Hz_old@x).squeeze()

print(torch.stack([Pz_fd, Pz_an, Pz_fd/Pz_an, Pz_an_old]))
