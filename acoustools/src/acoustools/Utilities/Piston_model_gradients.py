...
def compute_gradients(points, transducers = TRANSDUCERS, p_ref = Constants.P_ref, k=Constants.k, transducer_radius = Constants.radius, transducer_norms=None):
    '''
    @private
    ...
    if type(k) == float or type(k) == int:
        k = torch.ones(1,M,1, device=device, dtype=DTYPE) * k
    ...
    #Grad of e^ikd(xt,t)
    partialHpartialX = 1j * k_expanded * (diff / dist_expand) * torch.exp(1j * k_expanded * dist_expand)

    #Combine
    bessel_arg=k*transducer_radius*sin_theta
    F=1-torch.pow(bessel_arg,2)/8+torch.pow(bessel_arg,4)/192
    F = torch.unsqueeze(F,2)
    F = F.expand((-1,-1,3,-1))

    G = p_ref.unsqueeze(3) / dist_expand
    H = torch.exp(1j * k_expanded * dist_expand)
...
def forward_model_second_derivative_unmixed(points:Tensor, transducers:Tensor|None = None, p_ref = Constants.P_ref, k=Constants.k, transducer_radius=Constants.radius, transducer_norms=None) ->Tensor:
    '''
    ...
    kr_expand = k_expanded * transducer_radius
    ...
    da = -1 * diff / distances_expanded #Gradient of distances
    kd = k_expanded * distances_expanded
    ...
    daa = distance_bs / distances_expanded_cube #Second Gradient of distance function
    ...
    Ha = 1/48 * kr_expand**2 * sin_theta_expand * sa * (kr_expand**2 * sin_theta_expand**2 - 12)

    saa = daa * cos_theta_exp - da**2 * sin_theta_expand
    # saa[saa.isnan()] = 0

    Haa = 1/48 * kr_expand**2 * (3*sa**2 * (kr_expand**2 * sin_theta_expand_square- 4) + sin_theta_expand*saa * (kr_expand**2*sin_theta_expand_square - 12))
...
def forward_model_second_derivative_mixed(points: Tensor, transducers:Tensor|None = None, p_ref = Constants.P_ref,  k=Constants.k, transducer_radius=Constants.radius, transducer_norms=None)->Tensor:
    '''
    ...
    kd_exp = k_expanded * distances_expanded
    kd = k * distances
    ...
    Ha = 1/48 * kr_expanded**2 * sin_theta_expand * sa * (kr_expanded**2 * sin_theta_expand**2 - 12)
    ...
    Sxy = dxy * cos_theta - dx*dy*sin_theta
    Sxz = dxz * cos_theta - dx*dz*sin_theta
    Syz = dyz * cos_theta - dy*dz*sin_theta
