...
def stiffness_finite_differences(activations:Tensor, points:Tensor, board:Tensor|None=None, delta= 0.001):
    '''
    ...
    Fx1 = compute_force(activations,points + dx,board=board)[:,:,0]
    Fx2 = compute_force(activations,points - dx,board=board)[:,:,0]

    Fx = ((Fx1 - Fx2) / (2*delta))

    Fy1 = compute_force(activations,points + dy,board=board)[:,:,1]
    Fy2 = compute_force(activations,points - dy,board=board)[:,:,1]

    Fy = ((Fy1 - Fy2) / (2*delta))

    Fz1 = compute_force(activations,points + dz,board=board)[:,:,2]
    Fz2 = compute_force(activations,points - dz,board=board)[:,:,2]
